		...
		';
            ...
            function submit_form_data_ajax_handler()
            {
                ...
                $data = [
                    ...
                    'phone' => formatPhoneNumber($form_data['phone']),
                    'email' => sanitize_text_field($form_data['email']),
                    'comments' => sanitize_text_field($form_data['comments']),
                    'description' => $form_data['description'], 
                    'page_title' => $form_data['page_title'],
                    'url' => (!empty($form_data['url']) ? $form_data['url'] : ''),
                    'referrer' => $referrer,
                    'user_ip' => $user_ip,
                ...
                if (!is_wp_error($response)) {
                    ...
                    echo json_encode(['status' => 'error', 'message' => 'Произошла ошибка при отправке данных.', 'error_details' => [
                        'tools_lanet' => is_wp_error($response) ? $response->get_error_message() : null,
                    ]]);
                    exit();
                }
            }
